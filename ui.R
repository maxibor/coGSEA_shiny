    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Condition Plots",
                 ...
                 p("Fig 1 : Clustering of the GSEA methods on the genesets ranks (bases on raw p.values)"),
                 plotOutput("PCA"),
                 p("Fig 2 : PCA of the GSEA methods on the genesets ranks (bases on raw p.values)"),
                 plotOutput("eigen"),
                 p("Fig 3 : Fall of the eigen values for the PCA above"),
                 plotOutput("corPlot"),
                 p("Fig 4 : Correlation Plot of the GSEA methods on the genesets ranks (bases on raw p.values)"),
                 plotOutput("upsetr"),
                 p("Fig 5 : UpsetR Ensemblist plot showing the intersection of the different gene sets found significantly differentialy expressed (p.value < alpha) by each GSEA method"),
                 plotOutput("snailplot"),
                 p("Fig 6 : Mset Ensemblist plot showing the intersection of the different gene sets found significantly differentialy expressed (p.value < alpha) by each GSEA method"),
                 plotOutput("heatmap"),
                 p("Fig 7 : Binary heatmap showing wether a geneset is found differentially (red) expressed (p.value < alpha) by a GSEA method or not (blue)"),
