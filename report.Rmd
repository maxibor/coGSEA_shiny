...
for (i in colnames(contrast())){
  ...
  plot(resOutput()[["clustering"]][[i]],hang = -0.1)  
    cat("  \n")  
    cat("Fig 1 : Clustering of the GSEA methods on the genesets ranks (bases on raw p.values)  \n")  
  ...
  plot(res.pca)  
    cat("  \n")  
    cat("Fig 2 : PCA of the GSEA methods on the genesets ranks (bases on raw p.values)  \n")  
  ...
  corrplot(resOutput()$correlation[[i]], method = "circle", type = "full", order = "hclust",mar=c(10, 4, 4, 2) + 0.1)  
    cat("  \n")  
    cat("Fig 4 : Correlation Plot of the GSEA methods on the genesets ranks (bases on raw p.values)  \n")  
  
  upset(fromList(resOutput()$snailPlot[[i]]),order.by = "freq")   
    cat("  \n")
    cat("Fig 5 : UpsetR Ensemblist plot showing the intersection of the different gene sets found significantly differentialy expressed (p.value < alpha) by each GSEA method  \n")
  ...
  plot.msets(snail, sort.by="size", keep.empty.intersections=FALSE, min.intersection.size = input$min.intersection.size)    
    cat("  \n")  
    cat("Fig 6 : Mset Ensemblist plot showing the intersection of the different gene sets found significantly differentialy expressed (p.value < alpha) by each GSEA method  \n")  
  
  pheatmap::pheatmap(resOutput()$heatmap[[i]], fontsize = 8)  
    cat("  \n")  
    cat("Fig 7 : Binary heatmap showing wether a geneset is found differentially (red) expressed (p.value < alpha) by a GSEA method or not (blue)  \n")  
